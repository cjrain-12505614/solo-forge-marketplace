...
범용 1인 개발자 프로젝트 관리 플러그인 (v1.2.0)

## 개요

solo-forge는 개인 개발자가 **계획 → 구현 → 검증 → 회고를 한 바퀴 도는 1인 SDLC**를 CLAUDE.md 계약 위에서 운영하도록 돕는 플러그인입니다. 세션 관리·의사결정 추적·기능 기획·QA·핸드오프·진행 시각화 같은 **계획·문서 계층**에, TDD·검증 게이트·저장 후 셀프체크·파괴적 SQL 차단·git 일일 회고·정합성 교차검증 같은 **코드 실행 루프**를 더해, 혼자서도 놓치지 않고 한 사이클을 완주하게 합니다. 사내 시스템 의존 없이 어떤 프로젝트에도 도입할 수 있습니다.

### v1.0 — 무엇이 완성되었나

| 계층 | 담당 |
|---|---|
| **계획·문서** | project-bootstrap · session-protocol · decision-tracker · feature-planning · deliverable-management · handoff-protocol · status-reporter · lessons-learned |
| **코드 실행 루프** (v0.4~v1.0) | `/verify-step`(검증 게이트) · `/update-context`(컨텍스트 동기화) · tdd(RED→GREEN→REFACTOR) · code-quality 훅(저장 후 셀프체크) · git-daily(일일 회고) · consistency-check(정합성 교차검증) |
| **안전·위생 훅** | db-guard(파괴적 SQL 차단) · link-integrity(링크 무결성) · force-rules-inject(규칙 주입) · dev-gate(개발 중에만 넛지) |
| **시각화·산출** | progress-dashboard(진행 HTML) · deck-builder(편집 가능 PPTX) · figma-make-reviewer |
...
| tdd | RED→GREEN→REFACTOR 테스트 주도 개발 사이클 안내 (언어/러너 무관) (v0.5.0) |
| git-daily | git 커밋을 진행 로그에 자동 반영(읽기전용) + 커밋 안전 프로토콜(대량 삭제 방지) (v0.6.0) |
| consistency-check | 요구사항↔설계↔QA↔구현 추적키 grep 교차검증(독립 읽기전용 검증자) (v0.6.0) |
| deck-builder | 편집 가능 PPTX 발표자료 생성 (pptxgenjs 함정 회피·시각 QA, 중립 다크 기본 스타일) (v1.0.0) |
| mcp-inventory | 현재 세션 활성 MCP 도구를 서버별 인벤토리 보고 + 활성화 안내 (v1.1.0) |
| residual-audit | 워크스페이스 전체 .md 깨진 링크·depth 정합 광역 점검 + 리포트 (v1.1.0) |
| task-manager | 진행·예정·블로커·충돌 관점의 세션 지속 작업 보드(tasks.md) (v1.1.0) |
| dev-design | 코딩 전 개발설계 4종(로직·데이터모델·데이터접근·API계약) 확정 + 검증 게이트 (v1.2.0) |
| history-manager | "왜 이렇게 만들었나" 차수·이슈별 개발 맥락 보존 (v1.2.0) |
| document-curator | 유입 문서 파일명 표준화·버전 묶기·삭제후보 분류(근거 제시, 직접 삭제 안 함) (v1.2.0) |
| excel-reviewer | 엑셀 워크북 구조 정밀 점검(숨김 시트·병합·검증, zip/xml fallback) (v1.2.0) |
...
- **solo-code-quality-hook** (PostToolUse, v0.5.0) — 코드 파일 저장 후 품질 체크리스트 + 5회 수정마다 `/solo-verify-step` 권고 + 보안 민감 파일 알림. **기본 꺼짐** — 프로젝트 루트에 `touch .solo-forge` 하거나 `SOLO_DEV_HOOKS=1` 로 켜면 "개발 중인 프로젝트에서만" 울려 노이즈를 억제한다(게이트: `solo-dev-gate.sh`).
